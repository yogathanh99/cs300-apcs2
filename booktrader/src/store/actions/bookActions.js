...
const TRADE_URL_API = `http://${URL_SERVER}:${PORT_SERVER}/transactions`;
...
export const lendBook = (val, id) => async dispatch => {
  dispatch({ type: actionTypes.TRADE_BOOKS_START });
  await axios
    .post(TRADE_URL_API, val, {
      headers: {
        'Content-Type': 'application/json',
        'x-authenticated-uid': id,
      },
    ...
    .then(function(response) {
      console.log(response);
      dispatch({
        type: actionTypes.TRADE_BOOKS_SUCCESS,
      });
    })
    .catch(function(err) {
      console.log(err);
      dispatch({ type: actionTypes.TRADE_BOOKS_FAIL, payload: err.message });
    });
};
